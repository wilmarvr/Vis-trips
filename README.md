...
public/                # Frontend + PHP API
  ├─ index.html        # HTML die de app laadt
  ├─ styles.css        # Losse stylesheet
  ├─ app.js            # Losse JS voor API-calls en UI-updates
  ├─ api.php           # Backend eindpunt
  └─ config.sample.php # Optioneel: kopieer naar config.php voor DB-gegevens
database/
  └─ schema.sql        # Handige MySQL schema-export
.env.example           # Voorbeeld van DB variabelen (voor hosting die env vars ondersteunt)
```

## MySQL handmatig voorbereiden (optioneel)
De PHP backend maakt de tabel aan, maar je kunt het ook zelf doen via `database/schema.sql`:
```
